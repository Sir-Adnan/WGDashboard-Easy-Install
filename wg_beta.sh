...
setup_backup_bot() {
    ...
    apt-get update -qq >/dev/null 2>&1; apt-get install -y -qq tar xz-utils curl cron >/dev/null 2>&1
...
ARCHIVE_FILE="${TEMP_DIR}/${FILENAME}.tar.xz"
mkdir -p "${BACKUP_DIR}"
...
cd "${TEMP_DIR}"
tar -cJf "${ARCHIVE_FILE}" "${FILENAME}" >/dev/null 2>&1
...
restore_backup() {
    ...
    mapfile -t BACKUPS < <(ls /root/*.tar.xz 2>/dev/null)
    if [ ${#BACKUPS[@]} -eq 0 ]; then
        msg_box "err" "No .tar.xz files found in /root/ directory."
        read -p "   Press Enter..."; return
    fi
    ...
    if [ "$CONFIRM" == "restore" ]; then
        ...
        apt-get install -y -qq tar xz-utils >/dev/null
        ...
        tar -xf "$FILE_FULL_PATH" -C "$TEMP_RESTORE"
...
while true; do
    ...
    print_item 5 "Restore Data" "Restore from .tar.xz File"
